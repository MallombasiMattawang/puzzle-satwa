...
$.fn.jqPuzzle = function(settings, texts) {
	...
	var i18n = {
		en: {
			shuffleLabel: 			'MULAI',
			toggleOriginalLabel: 	'CEK',
			toggleNumbersLabel: 	'BANTUAN',
			confirmShuffleMessage: 	'YAKIN MULAI PERMAINAN ?',
			movesLabel:				'PERGERAKAN',
			secondsLabel:			'WAKTU'
		},
		fr: {
		...
		$srcImg.one('load', function() {
			...
			var finishGame = function() {
				if(success.fadeOriginal) {
					...
					if(control.toggleNumbers) $numbersButton.addClass('jqp-disabled');
					alert('BERHASIL!!!');
					$("#sukses").append('<div class="alert alert-success fade show"> <span class="close" data-dismiss="alert">×</span>  <strong>Berhasil!</strong>   <a href="1.html" class="btn btn-info btn-sm">Gambar Selanjutnya ?</a>. </div>' +
					'<ul>' +
					'<li> Nama latin	:	Macrocephalon maleo S.Muller, 1846</li>' +
					'<li> Nama Inggris	:	Maleo</li>' +
					'<li> Nama Lokal	:	Maleo, sengkawor, sengkawur, Maleoson(Minahasa), Molo(Sulteng)</li>' +
					'<li> Status Konservasi	:	IUCN : Genting / Endangered(versi 3.1), CITES : Appendix II, PP  no.7 th 1999 : Dilindungi pemerintah RI</li>' +
					'<li> Burung ini memiliki bulu berwarna hitam, kulit sekitar mata berwarna kuning, iris mata merah kecoklatan, kaki abu-abu, paruh jingga dan bulu sisi bawah berwarna merah-muda keputihan	.</li>' +
					'<li> Di atas kepalanya terdapat tanduk atau jambul keras berwarna hitam.Jantan dan betina serupa.. </li>' +
					'<li> Biasanya betina berukuran lebih kecil dan berwarna lebih kelam dibanding burung jantan.</li>' +
					'<li> Maleo Senkawor adalah monogami spesies.</li>' +
					'<li> Maleo menghabiskan sebagian besar waktunya mencari makanan dipermukaan tanah dengan mengais-ngais tanah</li>' +
					'<li> Maleo lebih suka berlari bersembunyi di tetymbuhan daripada terbang ketika ada bahaya</li>' +
					'<li> proses reproduksi atau perpanjangan keturunannya. Burung maleo yang bertelur itu tidak kemudian mengerami telurnya, ia lebih memilih mengubur telurnya di dalam pasir yang memiliki panas geothermal alami hingga menetas. Cara unik ini dilakukannya karena telur burung maleo itu sangatlah besar jika dilihat dari bentuk tubuhnya yang terhitung kecil. Ukuran telur burung maleo mampu memiliki ukuran 5 kali lebih besar dari ukuran telur ayam, bahkan karena saking besarnya telur burung maleo, ia sampai harus mengalami pingsan karena proses bertelur tersebut.</li>' +
					'<li>Pakan burung ini terdiri dari aneka biji-bijian, buah, semut, kumbang serta berbagai jenis hewan kecil </li>' +
					'<li> </li>' +
